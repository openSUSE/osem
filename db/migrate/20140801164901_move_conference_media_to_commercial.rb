...
class MoveConferenceMediaToCommercial < ActiveRecord::Migration
  ...
  def change
    ...
    TempConference.all.each do |conference|
      next if TempCommercial.exists?(commercialable_id: conference.id, commercialable_id: 'Conference')
      next if conference.media_id.blank? || conference.media_type.blank?
